...
function create_through_server(req, res) {
  ...
  cloudinary.uploader.upload(imageFile, { "tags": "resized", "width": 500, "height": 500, "crop": "fit" },)
    .then(function (image) {
    ...
    .finally(function () {
      ...
      res.end(JSON.stringify({ photo: photo, url: photo.image.url }, null, 3));
    });
}
...
function create_direct(req, res) {
  ...
  photo.save().then(function (photo) {
    ...
    .catch(function (err) {
      ...
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ photo: photo, upload: photo.image }, null, 3));
    });
}
